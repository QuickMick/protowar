...
{
    public class ProtoCamera : ISceneComponent
    {
        private static GraphicsDevice _graphics;

        public float SmoothSpeedMultiplicator { get; set; }
        ...
        /// </summary>
        public void Update(SceneContext e, GameTime gameTime)
        {
            if (_trackingBody != null)
            {
            ...
            {
                delta /= distance;
                delta *= SmoothSpeedMultiplicator;
            }
            float inertia;
            if (distance < 10f)
            {
        ...
        public void UnloadContent(IResourceManager scene)
        {
        }
        ...
        {
            SmoothSpeedMultiplicator = 1;
            _graphics = context.GraphicsDeviceManager.GraphicsDevice;
            SimProjection = Matrix.CreateOrthographicOffCenter(0f, ConvertUnits.ToSimUnits(_graphics.Viewport.Width), ConvertUnits.ToSimUnits(_graphics.Viewport.Height), 0f, 0f, 1f);
            SimView = Matrix.Identity;
            View = Matrix.Identity;
